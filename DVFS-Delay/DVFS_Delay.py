...
FreqMeasurementsFile=Path("FreqMeasurements.csv").resolve()
...
df.groupby(["Num_iterations","PE","Freq","NextFreq"]).sum(['TimeFreq1','TimeTotal']).reset_index()
...
def Calc_Delay(freq1=3, freq2=5, PE="Little", num_iterations=20000000):
    if freq1==freq2:
        return 0
    T1=df[(df['Num_iterations']==num_iterations) & (df['PE']==PE) & (df['Freq']==freq1) &(df['NextFreq']==freq1)]["TimeFreq2"].iloc[0]
    T2=df[(df['Num_iterations']==num_iterations) & (df['PE']==PE) & (df['Freq']==freq2) &(df['NextFreq']==freq2)]["TimeFreq2"].iloc[0]
    T1_2=df[(df['Num_iterations']==num_iterations) & (df['PE']==PE) & (df['Freq']==freq1) &(df['NextFreq']==freq2)]["TimeFreq2"].iloc[0]
...
pivoted = df.pivot_table(index=['Num_iterations','PE'], columns=['Freq','NextFreq'], values=['TimeFreq1','TimeTotal'])
print(pivoted)
